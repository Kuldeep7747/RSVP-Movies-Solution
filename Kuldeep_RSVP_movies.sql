...
-- Drama genre has highest movies produced overall --  4285
...
-- no of movies with one genre only are 3289
...
-- Thriller genre with 1484 movies and rank 3 
...
-- There are 361 movies,released between 1 April 2018 and 1 April 2019 with median rating of 8.
...
movie_income AS (
    SELECT m.id, m.title, m.year, g.genre,
           CASE 
               WHEN m.worlwide_gross_income LIKE 'INR%' 
               THEN CAST(REPLACE(REPLACE(m.worlwide_gross_income, 'INR', ''), ',', '') AS DECIMAL(12,2)) / 75
               WHEN m.worlwide_gross_income LIKE '$%' 
               THEN CAST(REPLACE(REPLACE(m.worlwide_gross_income, '$', ''), ',', '') AS DECIMAL(12,2))
               ELSE 0 
...
ranked_movies AS (
    SELECT genre, year, title AS movie_name, 
           ...
           RANK() OVER (PARTITION BY year, genre ORDER BY worldwide_gross_income DESC) AS movie_rank
    FROM movie_income
...
FROM ranked_movies
WHERE movie_rank <= 5
ORDER BY genre, year DESC, worldwide_gross_income DESC;
